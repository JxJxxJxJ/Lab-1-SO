...
void execute_pipeline(pipeline apipe){
    ...
    if(!pipeline_is_empty(apipe)){
        ...
        // file descriptors y temporales, para conectar las salidas con las entradas.   
        int fds[2];
        int temp[2];
        ...
        for(int i=0; i < pl_length;i++){
            ...
            if (i > 0) {
                // guardo los file descriptors del proceso en temp para luego reconectarlos con el siguiente proceso
                // (no lo hago si soy el primer proceso)
                temp[0] = fds[0];
                temp[1] = fds[1];
            }
